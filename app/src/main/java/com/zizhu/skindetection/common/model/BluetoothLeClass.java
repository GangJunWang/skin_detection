...
public class BluetoothLeClass{
	...
	public BluetoothLeClass(Context c,Handler handler){
    ...
    private final BluetoothGattCallback mGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                ...
                Log.e(TAG, "999999999999999999999999999Connected to GATT server."+cout++);
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt,
                                            BluetoothGattCharacteristic characteristic) {
        	if (mOnDataAvailableListener!=null)
                Log.e("bluetoth","11111111111111111111111改变了"+characteristic.getValue().toString());
    ...
    public void readCharacteristic(BluetoothGattCharacteristic characteristic) {
        ...
        Log.e("bluetooth", "```````````````````````````read:" + String.valueOf(mBluetoothGatt.readCharacteristic(characteristic)));
    }
    ...
    public void writeCharacteristic(BluetoothGattCharacteristic characteristic){
        ...
        Log.e("json", Utils.bytesToHexString(characteristic.getValue()));
        boolean status=mBluetoothGatt.writeCharacteristic(characteristic);

        Log.e("bluetooth", "````````````````````````````````````write:" + String.valueOf(status));
    }
    /**
    ...
    public List<BluetoothGattService> getSupportedGattServices() {
        ...
        return mBluetoothGatt.getServices();//获取到设备中的服务列表
    }
